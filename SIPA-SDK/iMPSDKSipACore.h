...
{
	iMPSdkSipA_EXOSIP_INIT_SUCCESS = ERRORCODE_BEGIN,
	iMPSdkSipA_EXOSIP_MALLOC_FAILED,      //exosip结构体分配内存失败
	iMPSdkSipA_EXOSIP_INIT_FAILED,        //Exosip协议栈初始化失败
	iMPSdkSipA_INTI_CLEAR_AUTHENTICATION_FAILED, //初始化时，清除鉴权消息失败
	iMPSdkSipA_INIT_BUILD_REGISTER_FAILED,
	iMPSdkSipA_INTI_SEND_REGISTER_FAILED,        //发送注册消息失败
	iMPSdkSipA_INIT_PLATFORM_TYPE_ERROR,     //sip服务器平台类型设置错误
	iMPSdkSipA_INIT_REGISTE_FAILED,         //注册失败

	iMPSdkSipA_PLAY_MEDIA_TRANSPRO_ERROR,  //实时点播时，设置媒体流传输协议错误
	iMPSdkSipA_PLAY_TCP_SETUP_ERROR,  //实时点播时，当媒体采用TCP方式传输时，发起端、接收端设置错误
	iMPSdkSipA_PLAY_BUILD_INVITE_FAILED,  //实时点播时，构建invite消息失败
	iMPSdkSipA_PLAY_SET_INVITE_BODY_FAILED, //实时点播时，设置invite消息体失败
	iMPSdkSipA_PLAY_SEND_INVITE_FAILED,   //实时点播时，发送invite消息失败

	iMPSdkSipA_STOP_GET_CID_FAILED,       //停止点播时，根据媒体端口号获取cid失败
	iMPSdkSipA_STOP_SEND_BYE_FAILED,      //停止点播时，发送bye消息失败

	iMPSdkSipA_CLOSECLIENT_BUILD_REGISTER_FAILED,
	iMPSdkSipA_CLOSECLIENT_SEND_REGISTER_FAILED,  //关闭sdk采用标准方式注销时，发送注销消息失败
	iMPSdkSipA_CLOSECLIENT_UNREGISTER_FAILED,     //关闭sdk采用标准方式注销时，失败
};

/***************SIP服务器类型*********************/
enum iMPSdkSipA_PLATFORM
{
	iMPSdkSipA_YSPLATFORM = 0,  //一所平台
	iMPSdkSipA_OTHERPLATFORM = 1, //其他平台
};
